...
def train_function(model, train_dataloader, val_dataloader, test_dataloader, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if best_val_loss > val_loss:
            ...
            checkpoint_path = os.path.join(CHECKPOINTS_DIR, f"best_epoch_{epoch}_{args.feat_extractor}_{args.dataset_name}_{args.task_type}_stff{args.stff_net_flag}")
...
def main():
    ...
    test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=args.num_workers)

    ###========get model to be finetuned========###
    if args.task_type == "ef_classification":
        model = VideoBinaryClassifier(args.feat_extractor, args.pretrained_checkpiont_path, args.stff_net_flag)
    elif args.task_type in ["ef_regression", "esv_regression", "edv_regression"]:
        model = VideoRegressor(args.feat_extractor, args.pretrained_checkpiont_path, args.stff_net_flag)
